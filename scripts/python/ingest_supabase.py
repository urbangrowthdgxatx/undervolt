...
def detect_energy_type(description):
    """Detect energy type from permit description."""
    if not description:
        return None
...
def update_aggregates(conn):
    ...
    cur.execute("""
        ...
        SELECT
            ...
            count(*) FILTER (WHERE energy_type = 'solar') as solar,
            count(*) FILTER (WHERE energy_type = 'battery') as battery,
            count(*) FILTER (WHERE energy_type = 'ev_charger') as ev_charger,
            count(*) FILTER (WHERE energy_type = 'generator') as generator,
            count(*) FILTER (WHERE energy_type = 'panel_upgrade') as panel_upgrade,
            count(*) FILTER (WHERE energy_type = 'hvac') as hvac,
