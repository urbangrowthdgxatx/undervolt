...
# Run with: sudo bash scripts/shell/setup_ollama_optimized.sh
...
echo ""
echo "Step 4: Warming up LLM model..."
...
echo "  curl -s http://localhost:11434/api/generate -d '{\"model\":\"llama3.2:3b\",\"prompt\":\"Hello\",\"stream\":false,\"keep_alive\":\"30m\",\"options\":{\"num_predict\":20}}' | python3 -c \"import sys,json; d=json.load(sys.stdin); print(f'{d[\\\"eval_count\\\"]} tokens at {d[\\\"eval_count\\\"]/(d[\\\"eval_duration\\\"]/1e9):.1f} tok/s')\""
echo ""
